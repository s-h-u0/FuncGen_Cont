 ...
 * @param resolution 分解能 (12/14/16bit)
 * @param mode       変換モード (ワンショット/連続)
 * @param gain       PGA ゲイン (1/2/4/8)
 * @return true = 成功, false = 失敗
 */
...
/**
 * @brief データレジスタから ADC 生データを取得（Ready ビット待ち含む）
 * @param dev デバイスハンドル
 * @return 生データ (LSB 単位)、エラー時は INT32_MIN
 */
int32_t MCP3428_ReadADC(MCP3428_HandleTypeDef *dev);

/**
 * @brief 生データを mV 単位で変換して返す
 * @param dev デバイスハンドル
